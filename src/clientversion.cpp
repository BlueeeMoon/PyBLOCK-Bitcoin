...
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments, const bool base_name_only)
{
    ...
    if (!base_name_only) {
        static const auto ua_knots = []() -> std::string {
            const auto pos{CLIENT_BUILD.find(".genesis")};
            return "Genesis:" + CLIENT_BUILD.substr(pos + 6) + "/";
        }();
        ua += ua_knots;
    }
    return ua;
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/SatoshiNakamotoBitcoin/Genesis>";
